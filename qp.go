...
// Package qp implements quoted-printable and message header encoding as
...
func Decode(dst, src []byte) (n int, err error) {
	...
	for i := 0; i < len(src); i++ {
			...
			for {
				...
				case '=':
					if trimLen > 0 {
						trimLen += eolLen + 1
						eolLen = 0
						err = fmt.Errorf("qp: invalid bytes after =: %q", src[eol-trimLen+1:eol])
					} else {
						trimLen = eolLen + 1
						eolLen = 0
					}
		...
		case (c >= ' ' && c <= '~') || c == '\n' || c == '\r' || c == '\t':
			...
			n++
		default:
			return n, fmt.Errorf("qp: invalid unescaped byte 0x%02x in quoted-printable body", c)
		}
	}
...
func fromHex(b byte) (byte, error) {
	...
	return 0, fmt.Errorf("qp: invalid quoted-printable hex byte 0x%02x", b)
}
